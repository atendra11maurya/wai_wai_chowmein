...
export default function App() {
  ...
  // In edit mode, page controls must never navigate, open links, submit forms,
  // or trigger their normal visitor-facing actions. Editor controls opt in with
  // data-editor-control so Save, Exit, and inline field popovers still work.
  const blockVisitorActionsInEditMode = (event) => {
    if (!editMode) return
    const action = event.target.closest('a, button, input[type="submit"]')
    if (!action || action.closest('[data-editor-control="true"]')) return
  ...
  return (
    <div className="shell min-h-screen" onClickCapture={blockVisitorActionsInEditMode}>
      ...
      {editMode && (
        <div data-editor-control="true" className="floating-save-bar glass" style={{ background: 'rgba(255, 255, 255, 0.95)', border: '2px solid rgba(239, 68, 68, 0.5)', boxShadow: '0 0 20px rgba(239, 68, 68, 0.3)' }}>
